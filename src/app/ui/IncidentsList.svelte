...
<script lang="ts">
	...
	import {abbreviate_addr, format_amount, format_time} from '#/util/format';
	...
	const DM_ICON_SEND = mk_icon(SX_ICON_SEND);
	const DM_ICON_RECV = mk_icon(SX_ICON_RECV);
	const DM_ICON_ACC_CREATED = mk_icon(SX_ICON_ACC_CREATED);
	...
	const H_INCIDENT_MAP: {
		...
		async tx_out(g_incident) {
			...
			if(f_interpret) {
				const g_interpretted = await f_interpret(g_msg_amino.value as JsonObject, g_context);
		...
		async tx_in(g_incident) {
			...
			for(const g_msg_proto of a_msgs_proto) {
				...
				const f_interpret = H_INTERPRETTERS[g_msg_amino.type];
				if(f_interpret) {
					const g_interpretted = await f_interpret(g_msg_amino.value, g_context);
		...
		async account_created(g_event) {
			...
			return {
				...
				name: g_account.name,
				icon: mk_icon(SX_ICON_ACC_CREATED),
				pfp: g_account.pfp || '',
			};
		},
		...
		async account_edited(g_event) {
			...
			return {
				...
				subtitle: `${format_time_ago(xt_when)} / ${g_account.name}`,
				name: g_account.name,
				icon: mk_icon(SX_ICON_ACC_EDITED),
				pfp: g_account.pfp || '',
			};
		},
		...
		async signed_json(g_event) {
			...
			return {
				title: `Signed ${h_events.query_permits?.length? 'query permit': 'document'}`,
				subtitle: `${format_time_ago(xt_when)} / ${g_account.name}`,
				name: g_account.name,
