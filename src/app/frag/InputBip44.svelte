<script lang="ts">
	...
	$: {
		...
		if(bip44) {
			// parse path
			const m_bip44 = R_BIP_44.exec(bip44);
	...
	function backflow() {
		bip44 = `m/44'/${s_coin_type}'/${s_account}'/${s_change}/${s_address}`;
	}
	...
	function selection_levels(): [number, number] {
		...
		return [
			bip44.slice(0, i_hi).split('/').length - 1,
			bip44.slice(0, i_lo).split('/').length - 1,
		];
	}

	function select_level(i_level: number) {
		const a_levels = bip44.split('/');
		const i_lo = a_levels.slice(0, i_level).join('/').length + 1;
		let i_hi = bip44.indexOf('/', i_lo);
		if(-1 === i_hi) i_hi = bip44.length;
