<script lang="ts">
	...
	import {Snip20MessageConstructor, Snip2xToken} from '#/schema/snip-2x-const';
	...
	(async() => {
		...
		if(!a_permits.length) {
			...
			s_permit_title = `${a_permits.length} permit${1 === a_permits.length? ' grants': 's grant'} ${as_apps.size} app${1 === as_apps.size? '': 's'} with query permissions`;
		}
	...
	async function rotate_key() {
		...
		const g_exec = await Snip20MessageConstructor.generate_viewing_key($yw_account, contract, $yw_network as SecretNetwork, s_viewing_key);
