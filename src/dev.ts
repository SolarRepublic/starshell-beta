	...
	if(!d_chrome['storage']) {
		function polyfill_chrome_storage(si_area: chrome.storage.AreaName): chrome.storage.StorageArea {
			return {
				...
				async get(z_keys: string | string[]): Promise<Dict<JsonValue | undefined>> {
				...
				async set(h_set: Dict<JsonValue | undefined>): Promise<void> {
				...
				async remove(z_keys: string | string[]): Promise<void> {
					const a_keys = Array.isArray(z_keys)? z_keys: [z_keys];
